-- ===================================================================
-- SPACECROP - BANCO DE DADOS ORACLE
-- ===================================================================
...
CREATE TABLE TB_FAZENDA (
    ...
    CONSTRAINT chk_area_hectares
        CHECK (nr_area_hectares > 0),
...
CREATE TABLE TB_SETOR_PLANTIO (
    ...
    CONSTRAINT chk_setor_area
        CHECK (nr_area_hectares > 0),

    CONSTRAINT fk_setor_fazenda
...
CREATE TABLE TB_SENSOR_ORBITAL (
    ...
    CONSTRAINT chk_sensor_ativo
        CHECK (fl_ativo IN ('S', 'N')),

    CONSTRAINT fk_sensor_satelite
        FOREIGN KEY (id_satelite)
        REFERENCES TB_SATELITE(id_satelite),

    CONSTRAINT fk_sensor_tipo
        FOREIGN KEY (id_tipo_sensor)
        REFERENCES TB_TIPO_SENSOR(id_tipo_sensor)
);

-- ===================================================================
-- 07. LEITURA DE SATELITE
-- ===================================================================
...
CREATE TABLE TB_LEITURA_SATELITE (
    ...
    CONSTRAINT chk_anomalia
        CHECK (fl_anomalia IN ('S', 'N')),

    CONSTRAINT fk_leitura_sensor
        FOREIGN KEY (id_sensor_orbital)
        REFERENCES TB_SENSOR_ORBITAL(id_sensor_orbital),

    CONSTRAINT fk_leitura_fazenda
        FOREIGN KEY (id_fazenda)
        REFERENCES TB_FAZENDA(id_fazenda),

    CONSTRAINT fk_leitura_setor
        FOREIGN KEY (id_setor)
        REFERENCES TB_SETOR_PLANTIO(id_setor)
);
...
CREATE TABLE TB_TIPO_ALERTA (
    ...
    CONSTRAINT chk_severidade
        CHECK (ds_severidade IN ('BAIXA', 'MEDIA', 'ALTA', 'CRITICA')),

    CONSTRAINT chk_requer_acao
        CHECK (fl_requer_acao IN ('S', 'N'))
);
...
CREATE TABLE TB_ALERTA (
    ...
    CONSTRAINT chk_resolvido
        CHECK (fl_resolvido IN ('S', 'N')),

    CONSTRAINT fk_alerta_leitura
        FOREIGN KEY (id_leitura)
        REFERENCES TB_LEITURA_SATELITE(id_leitura),

    CONSTRAINT fk_alerta_tipo
        FOREIGN KEY (id_tipo_alerta)
        REFERENCES TB_TIPO_ALERTA(id_tipo_alerta),
...
CREATE TABLE TB_ALERTAS_JSON (
    ...
    CONSTRAINT chk_json_valid
        CHECK (ds_conteudo IS JSON)
);
...
CREATE INDEX IDX_LEITURA_FAZENDA
    ON TB_LEITURA_SATELITE (id_fazenda, dt_leitura);

CREATE INDEX IDX_LEITURA_ANOMALIA
    ON TB_LEITURA_SATELITE (fl_anomalia);

CREATE INDEX IDX_ALERTA_NAO_RESOLVIDO
    ON TB_ALERTA (fl_resolvido, dt_alerta);

CREATE INDEX IDX_LEITURA_SENSOR
    ON TB_LEITURA_SATELITE (id_sensor_orbital);
